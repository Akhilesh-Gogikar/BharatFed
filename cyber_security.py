...
DSL_PATH = '../../cyber_security_secureboost_dsl.json' #'hetero_logistic_regression/test_hetero_lr_train_job_dsl.json'
SUBMIT_CONF_PATH = '../../cyber_security_secureboost_conf.json'#'hetero_logistic_regression/test_hetero_lr_train_job_conf.json'
...
GUEST_DATA_SET = 'cyber_security_guest_data.csv'
HOST_DATA_SET = 'cyber_security_host1_data.csv'
EVAL_DATA_SET = 'cyber_security_eval_data.csv'
...
def upload(role):
    ...
    if role == GUEST:
        file_path = '{}'.format(GUEST_DATA_SET)
    elif role == EVAL:
        file_path = '{}'.format(EVAL_DATA_SET)
    else:
        file_path = '{}'.format(HOST_DATA_SET)
    json_info['file'] = file_path
    json_info['table_name'] = table_name.split('/')[-1]
    json_info['namespace'] = table_name_space.split('/')[-1]
...
def submit_job():
    ...
    eval_table_name, eval_namespace = generate_data_info(EVAL)
